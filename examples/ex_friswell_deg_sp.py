...
def example_matrix_friswell(x:np.ndarray):
    ...
    """
    evaluation = sp.csc_matrix(
        [[2, 0],
         [0, 2]]
    )
    derivative1 = sp.csc_matrix(
        [[x[0], -x[1]],
         [-x[1], -x[0]]]
    )
    derivative2 = sp.csc_matrix((2, 2))
...
if __name__ == '__main__':
    ...
    orig_eigenvalues, orig_eigenvectors = np.linalg.eigh(example[0].toarray()) # eigh since multiplicity == dof
    ...
    mass_matrix_derivatives = ed.DerivativesList(
        [sp.eye(2, format='csc'), sp.csc_matrix((2,2)), sp.csc_matrix((2,2))]
    )
    ...
    polarization_derivatives, polarized_eigenvalues_derivatives = ed.polarization_derivatives(
        eigenvalue_derivatives, initial_polarization, polarization_order, eigenvector_derivatives
    )
